...
load(file = 'cooked/responses.rda')
```

```{r}
n_respondents <- nrow(tbl_responses)
```
...
tbl_barrier_summary <- tbl_barrier %>% 
  group_by(barrier) %>% 
  summarise(
    n_learning = sum(learning)
    , n_implementation = sum(implementation)
  ) %>% 
  mutate(
    pct_learning = n_learning / n_respondents
    , pct_implementation = n_implementation / n_respondents
...
tbl_technique_summary <- tbl_techniques %>% 
  group_by(technique) %>% 
  summarise(n_use = sum(use)) %>% 
  mutate(
    pct_use = n_use / n_respondents
  )
